...
def main():
    ...
    # 評価
    # 最初の方を除く
    RMSE = np.sqrt(((testData[250:] - testY[250:]) ** 2).mean())
    NRMSE = RMSE/np.sqrt(np.var(testData[250:]))
    print('RMSE =', RMSE)
    print('NRMSE =', NRMSE)

    # # csvファイルに記録
    # with open(args.figure_save_path + 'reproduct_lr_lr_cs_rs_01.csv', 'a') as f:
    #     writer = csv.writer(f)
    #     stim = args.test_name[0]
    #     [leak1, leak2] = args.leaking_rate
    #     # [nodeNum] = args.N_x
    #     # reservoir_num = args.reservoir_num
    #     cs = args.csv_seed
    #     rs = args.reservoir_seed
    #     writer.writerow([stim, leak1, leak2, RMSE, NRMSE, cs, rs])
