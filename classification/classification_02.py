...
def makeFig2(flag, model, tLabel, tData, tDataStd, tY, rmse, nrmse, viewLen=2450,):
    ...
    ax1.set_title("Input", fontsize=20)
    ax1.plot(tLabel[-viewLen:], color='k', label="input", linewidth=0.7)
    # hideValue = [x * 5 + args.bias for x in args.test_value]
    # hideLabel = md.makeDiacetylData(hideValue, args.test_duration).reshape(-1, 1)
    # ax1.plot(hideLabel[-viewLen:], alpha=0.0)
    ax1.grid(linestyle=":")
    ...
    ax2.plot(tData[-viewLen:], color="k", label="correct", alpha=0.7, linewidth=1, linestyle=":")
    ax2.set_xlabel("frame")
    ax2.legend()
...
def main():
    ...
    # テストデータ
    inputLabel = {"10-5":1, "10-6":1, "10-7":1, "10-8":1, "10-9":1, "0":0} # エレガントじゃない
    testData = []
    testDataStd = []
    testValue = []
    ...
    testLabel = cp.array(md.makeDiacetylData(cp.asnumpy(testValue), TEST_DURATION).reshape(-1, 1))
    ...
    #### labelとdataを逆にする ####
    temp = (trainLabel, trainData)
    trainInput, trainGT = temp
    temp = (testLabel, testData)
    testInput, testGT = temp
    ...
    # 評価
    # 最初の方を除く
    # RMSE = cp.sqrt(((testInput[200:] - testY[200:]) ** 2).mean())
