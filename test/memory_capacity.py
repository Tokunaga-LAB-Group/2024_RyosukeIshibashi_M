...
if __name__ == '__main__':
    ...
    # 時系列出力データ生成
    delay = cp.arange(40)  # 遅延長
    d = cp.empty((T, len(delay)))
    for k in tqdm(range(len(delay))):
        for t in range(T):
    ...
    # Input
    inputLayer = InputLayer(train_U.shape[1], 128, inputScale=1)
    ...
    # Reservoir
    # reservoirLayer = ReservoirLayer(128, 256, nodeNum, 0.24, 0.8, cp.tanh, 1.0)

    resInput1 = InputLayer(inputLayer.outputDimention, 128, inputScale=1, seed=11)
    resRes1 = ReservoirLayer(resInput1.outputDimention, 128, nodeNum, 0.24, 0.8, cp.tanh, 1.0, seed=101)

    resInput2 = InputLayer(inputLayer.outputDimention, 128, inputScale=1, seed=12)
    resRes2 = ReservoirLayer(resInput2.outputDimention, 128, nodeNum, 0.24, 0.8, cp.tanh, 1.0, seed=102)

    # resInput3 = InputLayer(16, 16, inputScale=1, seed=13)
    # resRes3 = ReservoirLayer(16, 64, nodeNum, 0.3, 0.95, cp.tanh, 0.22, seed=103)

    reservoirLayer = ParallelReservoirLayer(inputLayer.outputDimention, 256, [(resInput1, resRes1), (resInput2, resRes2)])
    ...
    # Output
    outputLayer = OutputLayer(256, train_D.shape[1])
    ...
    # 生成するファイル名
    fname = "../output/20240716/memory_capacity_22.png"
    # 保存
